...
I have also made a few changes to my network structure. I have implimented VXLAN to connect my VMs and Containers via a new 10.0.100.0/24 network. However for ease of use, I wanted to also use DHCP for less critical applications, and I also wanted to be able to route out to the internet over that same network. 

Currently, Proxmox 9 does not support this for VXLAN as it does for the Simple zone, and Simple zones themselves do not provide layer-2 benifits either, so a custom solution must be created.
...
To keep things (more or less) simple, I have overwritten the SDN interface for `vnet1` at the top of my "sdn-custom" config file. each node creates a network bridge `vnet1` with MASQUERADE pointed towards the wireless interface with gefault gateway of `10.0.100.254/24`:
